package com.ydc.schedule.config;
...
import com.ydc.schedule.quartz.ScheduleTask;  
...
public class QuartzConfigration {  
    ...
    public MethodInvokingJobDetailFactoryBean detailFactoryBean(ScheduleTask task) {// ScheduleTask为需要执行的任务  
         ...
         *  是否并发执行 
         *  例如:
         *  	每5s执行一次任务，但是当前任务还没有在5s内执行完，就已经过了5s了， 
        ...
        /* 
         * goScanfileIsExist为需要执行的方法 
         * 通过这几个配置，告诉JobDetailFactoryBean我们需要执行定时执行ScheduleTask类中的sayHello方法 
         */  
        jobDetail.setTargetMethod("goScanfileIsExist");  
